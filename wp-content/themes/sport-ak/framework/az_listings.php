...
$azexo_azl_templates = array(
    'google_map_post' => esc_html__('Google Map post', 'sport-ak'),
);
...
$azexo_azl_fields = array(
    'azl_actions' => esc_html__('Listing: Actions', 'sport-ak'),
    'azl_edit_link' => esc_html__('Listing: Edit link', 'sport-ak'),
    'azl_delete_link' => esc_html__('Listing: Delete link', 'sport-ak'),
    'azl_claim_link' => esc_html__('Listing: Claim link', 'sport-ak'),
    'azl_abuse_link' => esc_html__('Listing: Abuse link', 'sport-ak'),
    'azl_favorite_link' => esc_html__('Listing: Favorite link', 'sport-ak'),
);
...
function azexo_azl_entry_field($output, $name) {
    ...
    switch ($name) {
        case 'azl_actions':
            return '<div class="azl-actions"><input id="azl-actions-' . $post->ID . '" type="checkbox" style="position: absolute; clip: rect(0, 0, 0, 0);"><div></div><label for="azl-actions-' . $post->ID . '">' . esc_html('Actions', 'sport-ak') . '</label><div class=dropdown>'
        ...
        case 'azl_edit_link':
            if ($post->post_author == get_current_user_id() || current_user_can('manage_options')) {
                return '<div class="azl-edit"><a href="' . esc_url(add_query_arg(array('azl' => 'edit', 'id' => $post->ID))) . '">' . esc_html__('Edit', 'sport-ak') . '</a></div>';
            }
            break;
        case 'azl_delete_link':
            if ($post->post_author == get_current_user_id() || current_user_can('manage_options')) {
                return '<div class="azl-delete"><a href="' . esc_url(add_query_arg(array('azl' => 'delete', 'id' => $post->ID))) . '">' . esc_html__('Delete', 'sport-ak') . '</a></div>';
            }
            break;
        case 'azl_claim_link':
            if ($post->post_author == 1) {
                $users = get_post_meta($post->ID, 'claim');
                return '<div class="azl-claim ' . (in_array(get_current_user_id(), $users) ? 'claimed' : '') . '"><a href="' . esc_url(add_query_arg(array('azl' => 'claim', 'id' => $post->ID))) . '">' . esc_html__('Claim', 'sport-ak') . '</a></div>';
            }
            break;
        case 'azl_abuse_link':
            return '<div class="azl-abuse"><a href="' . esc_url(add_query_arg(array('azl' => 'abuse', 'id' => $post->ID))) . '">' . esc_html__('Report spam, abuse, or inappropriate content', 'sport-ak') . '</a></div>';
            break;
        case 'azl_favorite_link':
            $users = get_post_meta($post->ID, 'favorite');
            if (in_array(get_current_user_id(), $users)) {
                return '<div class="azl-favorite remove"><a href="' . esc_url(add_query_arg(array('azl' => 'favorite', 'id' => $post->ID))) . '">' . esc_html__('Remove from favorites', 'sport-ak') . '</a></div>';
            } else {
                return '<div class="azl-favorite add"><a href="' . esc_url(add_query_arg(array('azl' => 'favorite', 'id' => $post->ID))) . '">' . esc_html__('Add to favorites', 'sport-ak') . '</a></div>';
            }
            break;
    }
    return $output;
...
function azexo_azl_dashboard_links($links) {
    ...
    if (class_exists('WC_Vendors')) {
        if (WCV_Vendors::is_vendor(get_current_user_id())) {
            ...
            $links = array_merge(array(
                array(
                    'url' => $shop_page,
                    'title' => esc_html__('My items', 'sport-ak'),
                ),), $links);
        }
    }
    ...
    if ($can_submit && $submit_page && WCV_Vendors::is_vendor(get_current_user_id())) {
        $links = array_merge(array(
            array(
                'id' => $submit_page,
                'url' => get_permalink($submit_page),
                'title' => esc_html__('Submit new item', 'sport-ak'),
            ),), $links);
    }
