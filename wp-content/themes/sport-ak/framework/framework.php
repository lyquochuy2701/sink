...
define('AZEXO_FRAMEWORK_VERSION', '1.21');
...
function azexo_admin_notices() {
    if (version_compare(phpversion(), '5.3.6', '<')) {
        ?>
        <div class="error">
            <p><?php esc_html_e('PHP version must be >= 5.3.6', 'sport-ak'); ?></p>
        </div>
        <?php
    }
}
...
function azexo_after_setup_theme() {
    load_theme_textdomain('sport-ak', get_template_directory() . '/languages');
...
function azexo_admin_scripts() {
    ...
    wp_localize_script('azexo-admin', 'azexo', array(
        ...
        'templates_configuration' => esc_html__('Templates configuration', 'sport-ak'),
        'fields_configuration' => esc_html__('Fields configuration', 'sport-ak'),
        'post_types_settings' => esc_html__('Post types settings', 'sport-ak'),
        'woocommerce_templates_configuration' => esc_html__('WooCommerce templates configuration', 'sport-ak'),
        'section_alert' => esc_html__('Please click "Ok" to confirm this info. Theme not provide CSS styles for all possible configurations in this section. Please use configurations only provided with demo content. Of course you can use it with complete freedom, but in this case it may be necessary to add CSS styles. You can save confirmation of this alert in "AZEXO Options > General settings".', 'sport-ak'),
...
function azexo_scripts() {
    ...
    wp_enqueue_script('azexo-fontend', get_template_directory_uri() . '/js/fontend.js', array('jquery'), AZEXO_FRAMEWORK_VERSION, true);
    wp_localize_script('azexo-fontend', 'azexo', array(
    ...
    if (file_exists(get_template_directory() . '/js/imagesloaded.pkgd.min.js')) {
        wp_enqueue_script('imagesloaded', get_template_directory_uri() . '/js/imagesloaded.pkgd.min.js', array('jquery'), AZEXO_FRAMEWORK_VERSION, true);
    }
    ...
    if (!empty($options['custom-js'])) {
        wp_add_inline_script('azexo-fontend', $options['custom-js']);
    }
}
    ...
    protected function comment($comment, $depth, $args) {
        ...
        <?php if ('div' != $args['style']) : ?>
            ...
            <div class="comment-data">
                <?php printf(wp_kses(__('<cite class="fn">%s</cite>', 'sport-ak'), array('cite' => array('class' => array()))), get_comment_author_link()); ?>
                <?php if ('0' == $comment->comment_approved) : ?>
                    <em class="comment-awaiting-moderation"><?php esc_html_e('Your comment is awaiting moderation.', 'sport-ak') ?></em>
                    <br />
                <?php endif; ?>
                <div class="comment-meta commentmetadata"><a href="<?php echo esc_url(get_comment_link($comment->comment_ID, $args)); ?>">
                        <?php
                        /* translators: 1: date, 2: time */
                        printf(esc_html__('%1$s at %2$s', 'sport-ak'), get_comment_date(), get_comment_time());
                        ?></a><?php edit_comment_link(esc_html__('(Edit)', 'sport-ak'), '&nbsp;&nbsp;', '');
                        ?>
                </div>
                <?php comment_text(get_comment_id(), array_merge($args, array('add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
                <div class="reply">
                    <?php comment_reply_link(array_merge($args, array('reply_text' => esc_html__('Reply', 'sport-ak'), 'add_below' => $add_below, 'depth' => $depth, 'max_depth' => $args['max_depth']))); ?>
                </div>
            </div>
            <?php if ('div' != $args['style']) : ?>
            </div>
...
if (function_exists('register_nav_menus')) {
    register_nav_menus(array(
        'primary' => esc_html__('Top primary menu', 'sport-ak'),
        'secondary' => esc_html__('Secondary menu', 'sport-ak'),
    ));
}
...
function azexo_widgets_init() {
    if (function_exists('register_sidebar')) {
        register_sidebar(array('name' => esc_html__('Right/Left sidebar', 'sport-ak'), 'id' => "sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Header sidebar', 'sport-ak'), 'id' => "header_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Middle sidebar', 'sport-ak'), 'id' => "middle_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Footer sidebar', 'sport-ak'), 'id' => "footer_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Dashboard sidebar', 'sport-ak'), 'id' => "dashboard_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
        register_sidebar(array('name' => esc_html__('Additional sidebar', 'sport-ak'), 'id' => "additional_sidebar", 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<div class="widget-title"><h3>', 'after_title' => '</h3></div>'));
    }
}
...
function azexo_before_list($post_type) {
    ...
    $orderby_options = array(
        'menu_order' => esc_html__('Default sorting', 'sport-ak'),
        'date' => esc_html__('Sort by newness', 'sport-ak'),
    );
    if (isset($options[$post_type . '_custom_sorting']) && is_array($options[$post_type . '_custom_sorting'])) {
    ...
    if (isset($options[$post_type . '_custom_sorting_numeric_meta_keys']) && is_array($options[$post_type . '_custom_sorting_numeric_meta_keys'])) {
        foreach ($options[$post_type . '_custom_sorting_numeric_meta_keys'] as $meta_key) {
            if (!empty($meta_key)) {
                $orderby_options[esc_attr($meta_key) . '-desc'] = sprintf(esc_attr__('Sort by %s: high to low', 'sport-ak'), esc_attr($meta_key));
                $orderby_options[esc_attr($meta_key) . '-asc'] = sprintf(esc_attr__('Sort by %s: low to high', 'sport-ak'), esc_attr($meta_key));
            }
        }
    }
                ...
                case 'result_count':
                    ?><p class="result-count"><?php
                        ...
                        if ($total <= $per_page || -1 === $per_page) {
                            printf(_n('Showing the single result', 'Showing all %d results', $total, 'sport-ak'), $total);
                        } else {
                            printf(_nx('Showing the single result', 'Showing %1$d&ndash;%2$d of %3$d results', $total, '%1$d = first, %2$d = last, %3$d = total', 'sport-ak'), $first, $last, $total);
                        }
                        ?></p><?php
                        break;
                    case 'ordering':
...
function azexo_paging_nav() {
    ...
    $links = paginate_links(array(
        ...
        'prev_text' => '<i class="prev"></i>' . '<span>' . esc_html__('Previous', 'sport-ak') . '</span>',
        'next_text' => '<span>' . esc_html__('Next', 'sport-ak') . '</span>' . '<i class="next"></i>',
    ));
...
function azexo_post_nav_full() {
    ...
    <nav class="navigation post-navigation-full clearfix">
        <div class="prev-post">
            ...
            include(locate_template(apply_filters('azexo_post_template_path', 'content.php', $template_name)));
...
function azexo_get_the_category_list($separator = '', $parents = '', $post_id = false) {
    ...
    if (empty($categories)) {
        /** This filter is documented in wp-includes/category-template.php */
        return apply_filters('the_category', esc_html__('Uncategorized', 'sport-ak'), $separator, $parents);
    }
    ...
    switch ($name) {
        ...
        case 'post_sticky':
            if (is_sticky() && is_home() && !is_paged())
                return '<span class="featured-post">' . esc_html__('Sticky', 'sport-ak') . '</span>';
            break;
        ...
        case 'post_comments_count':
            ...
            $comments = '<a href="' . esc_url(get_comments_link()) . '"><span class="count">' . $comment_count['total_comments'] . '</span><span class="label">' . esc_html__('comments', 'sport-ak') . '</span></a>';
        ...
        default:
            if (isset($options['meta_fields']) && in_array($name, array_filter($options['meta_fields']))) {
                ...
                if (in_array($slug, $taxonomies)) {
                    $term_list = get_the_term_list(get_the_ID(), $slug, '', '<span class="delimiter">,</span> ', '');
...
function azexo_entry_share() {
    ...
    print '<a class="twitter-share" target="_blank" href="https://twitter.com/home?status=' . rawurlencode(esc_attr__('Check out this article: ', 'sport-ak')) . rawurlencode(get_the_title()) . '%20-%20' . rawurlencode(esc_url(apply_filters('the_permalink', get_permalink()))) . '"><span class="share-box"><i class="fa fa-twitter"></i></span></a>';
    if (!empty($image)) {
...
function azexo_entry_date($echo = true, $post = null) {
    if (has_post_format(array('chat', 'status'), $post))
        $format_prefix = _x('%1$s on %2$s', '1: post format name. 2: date', 'sport-ak');
    else
        $format_prefix = '%2$s';

    $options = get_option(AZEXO_FRAMEWORK);
    $date = sprintf('<span class="date">' . (isset($options['post_date_prefix']) ? esc_html($options['post_date_prefix']) : '') . '<a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a></span>', esc_url(get_permalink($post)), esc_attr(sprintf(esc_html__('Permalink to %s', 'sport-ak'), the_title_attribute(array('echo' => false, 'post' => $post)))), esc_attr(get_the_date('c', $post)), esc_html(sprintf($format_prefix, get_post_format_string(get_post_format($post)), get_the_date('', $post)))
    );
...
function azexo_comment_date($echo = true, $comment = null) {
    ...
    $options = get_option(AZEXO_FRAMEWORK);
    $date = sprintf('<span class="date">' . (isset($options['post_date_prefix']) ? esc_html($options['post_date_prefix']) : '') . '<a href="%1$s" title="%2$s" rel="bookmark"><time class="comment-date" datetime="%3$s">%4$s</time></a></span>', esc_url(get_comment_link($comment)), esc_attr(sprintf(esc_html__('Permalink to %s', 'sport-ak'), the_title_attribute(array('echo' => false, 'post' => $comment)))
...
function azexo_breadcrumbs() {
    /* === OPTIONS === */
    $text['home'] = esc_html__('Home', 'sport-ak'); // text for the 'Home' link
    $text['category'] = esc_html__('Archive by Category "%s"', 'sport-ak'); // text for a category page
    $text['tax'] = esc_html__('Archive for "%s"', 'sport-ak'); // text for a taxonomy page
    $text['search'] = esc_html__('Search Results for "%s" Query', 'sport-ak'); // text for a search results page
    $text['tag'] = esc_html__('Posts Tagged "%s"', 'sport-ak'); // text for a tag page
    $text['author'] = esc_html__('Articles Posted by %s', 'sport-ak'); // text for an author page
    $text['404'] = esc_html__('Error 404', 'sport-ak'); // text for the 404 page
    ...
    if (is_home() || is_front_page()) {
        ...
        if (get_query_var('paged')) {
            if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
                echo ' (';
            echo esc_html__('Page', 'sport-ak') . ' ' . get_query_var('paged');
            if (is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author())
                echo ')';
        }
    ...
    if (!empty($date_to)) {
        ...
        <div class="time-left">
            ...
            <div class="time" data-time="<?php print date('Y/m/d H:i:s', $date_to); ?>">
                <div class="days"><span class="count"><?php print $days; ?></span><span class="title"><?php print esc_html__('day', 'sport-ak'); ?></span></div>
                <div class="hours"><span class="count"><?php print $hours; ?></span><span class="title"><?php print esc_html__('hrs', 'sport-ak'); ?></span></div>
                <div class="minutes"><span class="count"><?php print $minutes; ?></span><span class="title"><?php print esc_html__('min', 'sport-ak'); ?></span></div>
                <div class="seconds"><span class="count"><?php print $seconds; ?></span><span class="title"><?php print esc_html__('sec', 'sport-ak'); ?></span></div>
            </div>
        </div>
        <?php
    }
...
function azexo_infinite_scroll_js_options($options) {
    ...
    $options['loading']['msgText'] = '<em class="infinite-scroll-loading">' . esc_html__('Loading ...', 'sport-ak') . '</em>';
    $options['loading']['finishedMsg'] = '<em class="infinite-scroll-done">' . esc_html__('Done', 'sport-ak') . '</em>';
    return $options;
}
...
function azexo_get_dashboard_links() {
    ...
    $links = array(
        array(
            ...
            'title' => esc_html__('Log out', 'sport-ak'),
        )
    );
    $options = get_option(AZEXO_FRAMEWORK);
